...
// Example Usage
getAddressCoordinates("Mumbai, India").then((coords) => {
    if (coords) {
        console.log(`Latitude: ${coords.lat}, Longitude: ${coords.lng}`);
    }
});

console.log(getAddressCoordinates("Mumbai, India"));
...
export const getDistanceTime = async (origin, destination) => {
  ...
  // Example Usage
  const origin = { lat: 19.076, lng: 72.8777 }; // Mumbai, India
  const destination = { lat: 28.7041, lng: 77.1025 }; // Delhi, India
  
  getDistanceTime(origin, destination).then((result) => {
    if (result) {
      console.log(`Estimated Distance: ${result.distance} km`);
      console.log(`Estimated Time: ${result.time} minutes`);
    }
  });
  ...
  // Example Usage
  getAutoCompleteSuggestions("Mumbai").then((suggestions) => {
